...
This module reuses the environment, reward function and discrete action tables
from ``parking_rl.py``. DQN is intentionally limited to fixed discrete action
spaces; PPO/SAC handle the continuous and curriculum experiments.
"""
...
from parking_rl import CONFIG, CarParkingEnvMulti, DISCRETE_9_TABLE, DISCRETE_43_TABLE, ensure_output_dirs, set_seed
...
class ReplayBuffer:
    ...
    def add(self, state: np.ndarray, action: int, reward: float, next_state: np.ndarray, done: bool) -> None:
        self.data.append((np.asarray(state, dtype=np.float32).copy(), int(action), float(reward), np.asarray(next_state, dtype=np.float32).copy(), float(done)))
...
class DuelingQNetwork(nn.Module):
    def __init__(self, state_dim: int, action_dim: int, hidden_size: int = 256) -> None:
        super().__init__()
        self.encoder = nn.Sequential(nn.Linear(state_dim, hidden_size), nn.LayerNorm(hidden_size), nn.ReLU(), nn.Linear(hidden_size, hidden_size), nn.ReLU())
        self.value = nn.Sequential(nn.Linear(hidden_size, hidden_size // 2), nn.ReLU(), nn.Linear(hidden_size // 2, 1))
        self.advantage = nn.Sequential(nn.Linear(hidden_size, hidden_size // 2), nn.ReLU(), nn.Linear(hidden_size // 2, action_dim))
...
class DoubleDQNAgent:
    ...
    def act(self, states: Sequence[np.ndarray], deterministic: bool = False) -> List[int]:
        ...
        greedy = torch.argmax(self.online(tensor), dim=-1).cpu().numpy().astype(np.int64)
        return [random.randrange(self.action_dim) if (not deterministic and random.random() < eps) else int(a) for a in greedy]

    def observe(self, states: Sequence[np.ndarray], actions: Sequence[int], rewards: Sequence[float], next_states: Sequence[np.ndarray], dones: Sequence[bool]) -> None:
        for item in zip(states, actions, rewards, next_states, dones):
            self.replay.add(*item)
        self.environment_steps += 1

    def update(self) -> Optional[Dict[str, float]]:
        if len(self.replay) < max(self.cfg.batch_size, self.cfg.warmup_steps) or self.environment_steps % max(1, self.cfg.train_every) != 0:
            return None
        ...
        if self.gradient_steps % max(1, self.cfg.target_update_every) == 0:
            self.target.load_state_dict(self.online.state_dict())
        return {"loss": float(loss.detach().cpu()), "mean_abs_td_error": float(torch.mean(torch.abs(target - q)).item()), "grad_norm": float(grad_norm), "epsilon": float(self.epsilon())}

    def save(self, path: Path, episode: int, best_reward: float) -> None:
        torch.save({"algorithm": "DOUBLE_DQN", "episode": int(episode), "best_reward": float(best_reward), "environment_steps": self.environment_steps, "gradient_steps": self.gradient_steps, "config": self.cfg.__dict__, "online": self.online.state_dict(), "target": self.target.state_dict(), "optimizer": self.optimizer.state_dict()}, path)
...
def train(cfg: DQNConfig) -> Path:
    ...
    best_path = dirs["checkpoints"] / f"dqn_{mode.lower()}_best.pth"
    latest_path = dirs["checkpoints"] / f"dqn_{mode.lower()}_latest.pth"
    ...
    for episode in range(1, cfg.episodes + 1):
        ...
        while True:
            ...
            next_states, rewards, dones = env.step([table[index].copy() for index in action_indices])
        ...
        if episode == 1 or episode % 10 == 0:
            ...
            print(f"[DQN] ep={episode:5d} reward={total_reward:10.2f} mean50={mean_reward:10.2f} steps={episode_steps:4d} eps={agent.epsilon():.3f}{stat_text}")
    return latest_path
...
def evaluate(cfg: DQNConfig, checkpoint: Path, episodes: int = 20) -> Dict[str, float]:
    ...
    result = {"episodes": float(episodes), "mean_reward": float(np.mean(rewards)), "std_reward": float(np.std(rewards)), "mean_length": float(np.mean(lengths))}
...
def main() -> None:
    ...
    if args.checkpoint:
        ...
        print(f"Saved latest checkpoint to {train(cfg)}")
